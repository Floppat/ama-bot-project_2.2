...
class DB_Manager:
    ...
    async def get_PK(self, table: str, col_name: str, col_content: int | str):
        async with aiosqlite.connect(self.database) as con:
            return (await (await con.execute(f'SELECT id FROM {table} WHERE {col_name} = ?;',(col_content,))).fetchall())[0][0] # pyright: ignore[reportIndexIssue]
    ...
    async def ai_read(self, PK: int):
        async with aiosqlite.connect(self.database) as con:
            try:
                return json.loads((await (await con.execute(f'SELECT history FROM ai_history WHERE id = {PK};')).fetchall())[0][0]) # pyright: ignore[reportIndexIssue]
            except IndexError:
                return False
